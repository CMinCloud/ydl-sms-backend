...
public class SmsSendServiceImpl implements SmsSendService {
    ...
    private boolean auth;    //判断平台是否进行认证
    @Value("${ydlclass.sms.domain}")
    private String domain;   //设置接收端在哪里
    ...
    private R send(BaseParamsDTO baseParamsDTO, String url) {
        ...
        if (auth) {   //如果要认证的话
            //key secret 都要有
            if (StringUtils.isBlank(accessKeyId) || StringUtils.isBlank(accessKeySecret)) {  //isBlank是否为空
        ...
        try {
            ...
            HttpEntity responseEntity = response.getEntity();  //得到了响应

            //解析响应码
            if (response.getStatusLine().getStatusCode() == 200) {
                //发送成功
                log.info("httpRequest access success, StatusCode is:{}", response.getStatusLine().getStatusCode());
                String responseEntityStr = EntityUtils.toString(responseEntity);  //响应体的Str串
                log.info("responseContent is :" + responseEntityStr);

                return JSON.parseObject(responseEntityStr, R.class);  //将响应体字符串转义成R
            } else {
                //发送失败
            ...
            post.releaseConnection();   //将请求释放掉，关闭连接
        }
    }
}
